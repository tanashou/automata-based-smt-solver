...
def get_all_benchmark_list_files() -> list[Path]:
    """Get all .txt files from the paths directory and subdirectories.
    ...
    Searches both paths/LIA and paths/QF_LIA directories.
    """
    if not PATHS_BASE_DIR.exists():
        return []

    # paths/LIA/*.txt と paths/QF_LIA/*.txt の両方を収集
    all_files: list[Path] = []
    for category in ["LIA", "QF_LIA"]:
        category_dir = PATHS_BASE_DIR / category
        if category_dir.exists():
            all_files.extend(category_dir.glob("*.txt"))
...
def collect_all_benchmark_paths() -> list[tuple[str, str, Path]]:
    """Collect all benchmark paths from all list files.

    Returns a list of tuples: (category, benchmark_set_name, path_to_smt2_file)
    where category is "LIA" or "QF_LIA"
    """
    all_paths: list[tuple[str, str, Path]] = []
    benchmark_list_files = get_all_benchmark_list_files()

    for list_path in benchmark_list_files:
        # カテゴリ名を取得 (LIA または QF_LIA)
...
def test_solver_benchmark(benchmark, benchmark_info: tuple[str, str, Path]):
