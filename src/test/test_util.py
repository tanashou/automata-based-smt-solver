...
class TestBinaryStringsWithWildcard:
    def test_no_wildcards(self):
        mask = [True, True, True]
        expected_output = {"000", "001", "010", "011", "100", "101", "110", "111"}
        assert binary_strings_with_wildcard(mask) == expected_output
...
class TestDotProductWithWildcard:
    def test_all_values_no_wildcard(self):
        assert dot_product_with_wildcard(["1", "0", "1"], "101") == 2

    def test_with_wildcards(self):
        assert dot_product_with_wildcard(["1", "1", "1"], "1*1") == 2

    def test_all_wildcards(self):
        assert dot_product_with_wildcard(["1", "1", "1"], "***") == 0
